...
deforum_titles = {
    ...
    "Sampler": "Determines the algorithm used to generate the initial noise for the latent diffusion process.",
    "Scheduler": "Determines the algorithm used to generate the next sample in the latent diffusion process.",
    "Enable extras": "enable additional seed settings",
    "Subseed schedule": "Seed of a different picture to be mixed into the generation.",
    "Subseed strength schedule": "How strong of a variation to produce. At 0.0, there will be no effect. At 1.0, you will get the complete picture with variation seed (except for ancestral samplers, where you will just get something).",
    ...
    "Seed behavior": "defines the seed behavior that is used for animations",
        "iter": "the seed value will increment by 1 for each subsequent frame of the animation. (adding +1 to a seed creates a totally new one with no relation to the one before. Technically this is very similar to random, but more predictable.",
        ...
        "random": "a random seed will be used on each frame of the animation. **NOT REALLY RECOMMENDED.** for better reproducibility, consider using 'iter' instead.",
		"ladder": "2 steps up, then 1 step down again.",
		"alternate": "one step up, then one step down again. **NOT RECOMMENDED.**",
    ...
    // Note: Hybrid Video functionality removed - all hybrid hints eliminated
}
