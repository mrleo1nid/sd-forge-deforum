...
from deforum.utils.general import duplicate_pngs_from_folder, checksum
from deforum.media.video_audio_utilities import vid2frames, ffmpeg_stitch_video, extract_number, media_file_has_audio
...
def process_ncnn_video_upscaling(vid_path, outdir, in_vid_fps, in_vid_res, out_vid_res, models_path, upscale_model, upscale_factor, keep_imgs, f_location, f_crf, f_preset, current_user_os):
    ...
    ffmpeg_stitch_video(ffmpeg_location=f_location, fps=in_vid_fps, outmp4_path=out_upscaled_mp4_path, stitch_from_frame=1, stitch_to_frame=-1, imgs_path=upscaled_imgs_path_for_ffmpeg, add_soundtrack=add_soundtrack, audio_path=vid_path.name, crf=f_crf, preset=f_preset)
...
def check_and_download_realesrgan_ncnn(models_folder, current_user_os):
    ...
    try:
        ...
        download_url_to_file(download_url, realesrgan_zip_path)
