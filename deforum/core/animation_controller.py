...
def transform_image_3d_legacy(device, prev_img_cv2, depth_tensor, rot_mat, translate, anim_args, keys, frame_idx):
    ...
    # Create identity rotation and zero translation for old camera, ensuring they're on the correct device
    identity_R = torch.eye(3, device=device).unsqueeze(0)
    zero_T = torch.zeros((1, 3), device=device)
    persp_cam_old = p3d.FoVPerspectiveCameras(near, far, aspect_ratio, fov=fov_deg, degrees=True, R=identity_R, T=zero_T, device=device)
    persp_cam_new = p3d.FoVPerspectiveCameras(near, far, aspect_ratio, fov=fov_deg, degrees=True, R=rot_mat, T=torch.tensor([translate], device=device), device=device)
