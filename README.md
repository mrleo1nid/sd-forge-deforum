...
Deforum supports multiple animation workflows:

#### **2D / 3D Modes**
Traditional Deforum animation with optional Wan FLF2V integration:
- Generate keyframes with Flux/SD models
- Use depth warping (3D) or flat transformations (2D)
...
#### **Wan Only Mode** 🆕
Pure Wan workflow - 100% AI video generation:
- **Phase 1**: Generate ALL keyframes with Wan T2V
- **Phase 2**: Interpolate ALL tween frames with Wan FLF2V
- **Phase 3**: Stitch final video
- **No SD model required** - entire video generated by Wan
- Batch processing architecture for efficient generation
- Resume capability - can restart from Phase 2 if keyframes already exist

#### **Wan Flux Mode** 🆕
Hybrid workflow combining Flux and Wan:
- **Phase 1**: Generate keyframes with Flux
- **Phase 2**: Interpolate tweens with Wan FLF2V
- **Phase 3**: Stitch final video
- Best of both worlds: Flux image quality + Wan motion interpolation

#### **Video Input Mode**
Use existing video as initialization for transformations
...
#### Animation Modes
* **2D/3D Modes**: Traditional Deforum animation with depth warping
  * Optional Wan FLF2V integration for AI-powered tween interpolation
  * Keyframes Only or Cadence-based distribution
* **Wan Only Mode**: Pure Wan T2V + FLF2V batch processing (no SD model required)
  * Resume capability at Phase 2 (FLF2V interpolation)
  * Batch generation: All keyframes → All interpolations → Stitch
* **Wan Flux Mode**: Hybrid Flux keyframes + Wan FLF2V interpolation
  * Best of both: Flux quality + Wan motion
* **Video Input**: Use existing video as initialization
